# Strict PRD Grammar and Parser

**Overview:** Build the foundation of Zurdo, a Rust binary crate that reads a markdown PRD written under Zurdo's strict grammar (resolutions section 1.1), validates it structurally and semantically including skill-directory existence on disk, and reports errors with precise line numbers. Exposes a `zurdo validate` subcommand that exits 0 on a clean parse and 1 on any validation error, with human-readable diagnostics on stderr. This PRD intentionally stops short of `prd.json` persistence (PRD-03), config loading (PRD-02), and runtime execution (PRD-04). It also defers `effort_map` enum membership checks to PRD-02, since the parser does not load `.zurdo/config.toml`.

**Project Directory:** /Users/orlandogarcia/numeron/zurdo
...
### Acceptance Criteria
- `Cargo.toml` exists at the repo root with name set to zurdo, edition set to 2024, and dependencies on clap, thiserror, serde, serde_json, sha1, and regex
...
- A `clippy.toml` file exists at the repo root (may be empty) and `cargo clippy --all-targets -- -D warnings` exits cleanly
...
- The TaskId type is a newtype with a constructor that validates against the regex pattern for `task-` followed by lowercase alphanumerics and hyphens, and returns a Result with a structured parse error on rejection
- The Hint enum has exactly five variants: Shell with a cmd field, Http with method url and expected_status fields, FileExists with a path field, Grep with pattern and file fields, and Manual
- The Duration type parses values like `30m` to thirty minutes and like `1h` to one hour, but rejects bare integers like `30` with a structured parse error
- A `parse_hints` function lives in the parser module and is callable independently of full PRD parsing
- Calling `parse_hints` on a criterion line containing two bracket hints returns the human text without the brackets plus a vector of two hints in source order
...
- Unit tests cover all five hint types parsed correctly, multiple hints on one line preserved in source order, malformed hints (empty shell command, http hint missing the expected status) producing a structured ParseError, bare integer durations rejected, valid task ids accepted, and invalid task ids rejected
- Running `cargo test` passes with at least fifteen unit tests in this module
...
- A missing or duplicate H1 produces a structured error with the offending line number
- An H2 heading that does not match the required task heading format including the literal em-dash separator produces an error citing the line number
- A metadata key not in the closed enum produces an error naming the offending key and line number
- A blank line inside the metadata block ends the block, and any later bold-key colon-value lines are treated as prose and reported with the message that the metadata block must be contiguous under H2
...
- Each parsed Criterion carries the original line number for downstream error reporting
- Unit tests cover a minimal valid PRD with one task, PRDs exercising each error class above, and a PRD with two tasks where the second has all five optional metadata fields populated
...
- A dependency cycle (two-task or longer) produces a DependencyCycle error naming all tasks in the cycle
...
- Running `cargo run -- validate path` on an invalid PRD exits with code 1 and prints one human-readable diagnostic per error to stderr, each in the form file colon line colon message so editors can jump to it
...
- Fixture PRDs cover at minimum: valid single-task, valid multi-task with dependencies, duplicate ids, unknown dependency, self-dependency, two-task cycle, three-task cycle, criterion missing hints, and unknown skill reference
- The skill resolution test creates a temporary directory, writes a SKILL.md for one skill name, writes one PRD that references the existing skill and one PRD that references a missing skill, and asserts the first passes validation while the second fails with UnknownSkill
